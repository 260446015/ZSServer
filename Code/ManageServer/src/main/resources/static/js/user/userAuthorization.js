            ...
            if (res.success) {
                if(res.data.totalPage>0){
                    ...
                    for (var i = 0; i < arr.length; i++) {
                        ...
                        html += '<tr class="gradeX"><input type="hidden" class="form-control input-block" value="'+arr[i].id+'"/><td>' + arr[i].parkName + '</td><td>' + arr[i].area + '</td><td>' + arr[i].accountCount + '</td>'+
                            '<td class="actions">'+ '<a href="javascript:void(0);" class="on-default my_edit">查看详情</a></td></tr>';
                    }
                    $("#user_list").html(html);
...
function initPage() {
    $(".my_edit").on("click", function () {
        ...
        window.location.href = "/apis/user/accountDetail.html?id="+_id;
    });
