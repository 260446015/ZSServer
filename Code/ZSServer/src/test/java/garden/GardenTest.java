...
//		}
    ...
    public void getBaseInfo() throws Exception {
        WpsUtilsTest util = new WpsUtilsTest();
        FileInputStream is = new FileInputStream("C:\\Users\\yindawei\\Documents\\Tencent Files\\260446015\\FileRecv\\企业(1).xlsx");
        Map<Integer, String> content = util.readExcelContentXlsx(is, "&");
        OpeneyesDTO dto = new OpeneyesDTO();
        int errorCount = 0;
        ...
        for (Map.Entry<Integer, String> entry : content.entrySet()) {
            ...
            try {
                ...
                JSONObject result = openeyesService.getBaseInfo(dto);
                JSONObject baseInfo = result.getJSONObject("result");
                String legalPersonName = baseInfo.getString("legalPersonName");
                String regCapital = baseInfo.getString("regCapital");
                Long estiblishTime = baseInfo.getLong("estiblishTime");
                String formatDate = "";
                try {
                    formatDate = format.format(estiblishTime);
                }catch (Exception e){
                    System.out.println("格式转化失败");
                }
                JSONObject obj = new JSONObject();
                obj.put("name",arr[0]);
                obj.put("legalPersonName",legalPersonName);
                obj.put("regCapital",regCapital);
                obj.put("estiblishTime",formatDate);
                array.add(obj);
            } catch (Exception e) {
                errorCount++;
                log.info("失败名称为:" + dto.getCname());
