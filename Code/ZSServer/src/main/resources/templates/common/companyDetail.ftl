<h2 class="datails-title">
    	<span id="baseName"></span>
    <a href="javascript:void(0);" class="follow pull-right" id="attationCompany">添加关注</a>
</h2>
<div class="border-shadow-box content-padding-20">
    <h4 class="score-title" id="baseScore"></h4>
    <div class="lyt-box inline-lyt">
        <div class="lyt-item">
            <div class="lyt-lt">
                <span class="icon-block"></span>
                电话
            </div>
            <div class="lyt-rt" id="baseTel"></div>
        </div>
        <div class="lyt-item">
            <div class="lyt-lt">
                <span class="icon-block"></span>
                网址
            </div>
            <div class="lyt-rt" id="baseWeb"></div>
        </div>
        <div class="lyt-item">
            <div class="lyt-lt">
                <span class="icon-block"></span>
                地址
            </div>
            <div class="lyt-rt" id="baseAddr"></div>
        </div>
    </div>
</div>
<ul class="mark-box">
    <li>
        <a class="mark-item" href="javascript:void(0);">基本信息</a>
        <ul class="mark-menu">
            <li>
                <a href="/apis/company/staff.html?companyName=${companyName}">主要人员</a>
            </li>
        </ul>
    </li>
    <li>
        <a class="mark-item" href="javascript:void(0);">企业背景</a>
        <ul class="mark-menu">
            <li>
                <a href="/apis/company/inverst.html?companyName=${companyName}">对外投资</a>
            </li>
            <li>
                <a href="/apis/company/changeInfo.html?companyName=${companyName}">变更记录</a>
            </li>
            <li>
                <a href="/apis/company/branch.html?companyName=${companyName}">分支机构</a>
            </li>
        </ul>
    </li>
    <li>
        <a class="mark-item" href="javascript:void(0);">企业发展</a>
        <ul class="mark-menu">
            <li>
                <a href="/apis/company/historyRongzi.html?companyName=${companyName}">融资历史</a>
            </li>
            <li>
                <a href="/apis/company/teamMember.html?companyName=${companyName}">核心团队</a>
            </li>
            <li>
                <a href="/apis/company/productInfo.html?companyName=${companyName}">企业业务</a>
            </li>
            <li>
                <a href="/apis/company/touzi.html?companyName=${companyName}">投资事件</a>
            </li>
            <li>
                <a href="/apis/company/jingpin.html?companyName=${companyName}">竞品信息</a>
            </li>
        </ul>
    </li>
    <li>
        <a class="mark-item" href="javascript:void(0);">经营状况</a>
        <ul class="mark-menu">
            <li>
                <a href="/apis/company/bids.html?companyName=${companyName}">招投标</a>
            </li>
            <li>
                <a href="/apis/company/bond.html?companyName=${companyName}">债券信息</a>
            </li>
            <li>
                <a href="/apis/company/purchaseland.html?companyName=${companyName}">购地信息</a>
            </li>
            <li>
                <a href="/apis/company/employMents.html?companyName=${companyName}">招聘</a>
            </li>
            <li>
                <a href="/apis/company/taxCredit.html?companyName=${companyName}">税务评级</a>
            </li>
            <li>
                <a href="/apis/company/checkinfo.html?companyName=${companyName}">抽查检查</a>
            </li>
            <li>
                <a href="/apis/company/appbkInfo.html?companyName=${companyName}">产品信息</a>
            </li>
            <li>
                <a href="/apis/company/certificate.html?companyName=${companyName}">资质证书</a>
            </li>
            <li>
                <a href="/apis/company/lawSuit.html?companyName=${companyName}">法律诉讼</a>
            </li>
            <li>
                <a href="/apis/company/courtAnnouncement.html?companyName=${companyName}">法院公告</a>
            </li>
            <li>
                <a href="/apis/company/dishonest.html?companyName=${companyName}">失信人</a>
            </li>
            <li>
                <a href="/apis/company/zhixinginfo.html?companyName=${companyName}">被执行人</a>
            </li>
        </ul>
    </li>
    <li>
        <a class="mark-item" href="javascript:void(0);">知识产权</a>
        <ul class="mark-menu">
            <li>
                <a href="/apis/company/shangbiao.html?companyName=${companyName}">商标信息</a>
            </li>
            <li>
                <a href="/apis/company/patents.html?companyName=${companyName}">专利</a>
            </li>
            <li>
                <a href="/apis/company/copyReg.html?companyName=${companyName}">著作权</a>
            </li>
            <li>
                <a href="/apis/company/icp.html?companyName=${companyName}">网站备案</a>
            </li>
        </ul>
    </li>
    <li>
        <a class="mark-item" href="javascript:void(0);">上市信息</a>
        <ul class="mark-menu">
            <li>
                <a href="/apis/company/volatility.html?companyName=${companyName}">股票行情</a>
            </li>
            <li>
                <a href="/apis/company/companyInfo.html?companyName=${companyName}">企业简介（股票）</a>
            </li>
            <li>
                <a href="/apis/company/seniorExecutive.html?companyName=${companyName}">高管信息（股票）</a>
            </li>
            <li>
                <a href="/apis/company/holdingCompany.html?companyName=${companyName}">参股控股（股票）</a>
            </li>
            <li>
                <a href="/apis/company/announcement.html?companyName=${companyName}">上市公告（股票）</a>
            </li>
            <li>
                <a href="/apis/company/shareholder.html?companyName=${companyName}">十大股东（股票）</a>
            </li>
            <li>
                <a href="/apis/company/issueRelated.html?companyName=${companyName}">发行相关（股票）</a>
            </li>
            <li>
                <a href="/apis/company/shareStructure.html?companyName=${companyName}">股本结构（股票）</a>
            </li>
            <li>
                <a href="/apis/company/equityChange.html?companyName=${companyName}">股本变动（股票）</a>
            </li>
            <li>
                <a href="/apis/company/bonusInfo.html?companyName=${companyName}">分红情况（股票）</a>
            </li>
            <li>
                <a href="/apis/company/allotmen.html?companyName=${companyName}">配股情况（股票）</a>
            </li>
        </ul>
    </li>
</ul>
