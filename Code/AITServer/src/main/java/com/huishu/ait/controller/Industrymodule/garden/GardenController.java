...
public class GardenController extends BaseController {
	 ...
	 * @return
	 */
	@RequestMapping(value = "/findIndustryCountByArea.json", method = RequestMethod.GET)
	@ResponseBody
	public AjaxResult findIndustryCountByArea() {
		JSONArray data = null;
		try {
			data = gardenService.findGardensAll();
		} catch (Exception e) {
			LOGGER.error("查询各个地区上不同产业类型的个数失败", e.getMessage());
			return error("查询各个地区上不同产业类型的个数失败");
		}
		return success(data);
	}
